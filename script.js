...
const state = {
  ...
  function setupDateSelection() {
    ...
    // Update quick dates when input changes
    if (dateInput) {
...
function addCustomBoxToCart() {
  ...
  function validateForm() {
    ...
    const paymentSelect = DOM.get('select[name="payment"]');
    ...
    const contactNumber = DOM.get('input[name="contactNumber"]').value.trim();
    ...
    // Name validation
    const name = DOM.get('input[name="name"]').value.trim();
    if (name.length < 2) {
    ...
    const deliveryMethod = DOM.get('input[name="deliveryMethod"]:checked');
    if (!deliveryMethod) {
    ...
    const deliveryDate = DOM.get('input[name="deliveryDate"]').value;
    if (!deliveryDate) {
    ...
    const timeSlot = DOM.get('input[name="timeSlot"]:checked');
    if (!timeSlot) {
  ...
  function validateSocialMedia() {
    const socialHandle = DOM.get('#socialHandleInput').value.trim();
    const facebookChecked = DOM.get('#platformFacebook').checked;
    const instagramChecked = DOM.get('#platformInstagram').checked;
    const tiktokChecked = DOM.get('#platformTiktok').checked;
    
    if (socialHandle && !facebookChecked && !instagramChecked && !tiktokChecked) {
...
function initializeHomepage() {
  initializeFlavorsSection(); // Updated this line
  ...
  function displayOrderDetails() {
    ...
    if (orderData && orderData.orderId) {
      ...
      DOM.get('#itemCount').textContent = (orderData.itemCount || 0) + ' item(s)';
